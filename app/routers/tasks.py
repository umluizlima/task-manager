...
def list(db: Session = Depends(get_db)):
    return sorted(
        TasksRepository(db).find_all(), key=lambda task: task.status.value, reverse=True
    )
...
def delete(task_id: UUID, db: Session = Depends(get_db)):
    try:
        return TasksRepository(db).delete_by_id(task_id)
    except Exception:
...
def read(task_id: UUID, db: Session = Depends(get_db)):
    try:
        return TasksRepository(db).find_by_id(task_id)
    except Exception:
        raise HTTPException(status_code=404, detail="Task not found")
...
def update(task_id: UUID, task: TaskUpdate, db: Session = Depends(get_db)):
    try:
        return TasksRepository(db).update_by_id(task_id, task.dict(exclude_unset=True))
    except Exception:
        raise HTTPException(status_code=404, detail="Task not found")
